...
export function getWhatsAppStatus(rawAccountId?: string | null): WhatsAppBotStatus {
  ...
  if (!instance.status.isConnected && !instance.status.isConnecting && !instance.sock) {
    if (hasExistingSession(accountId)) {
      // Auto-reconnect in background if local session files are present
      connectWhatsAppBot(accountId).catch(() => {});
      return {
        ...instance.status,
        accountId,
        isConnecting: true,
...
export async function connectWhatsAppBot(
  ...
  if (!forceNewQR && instance.connectingPromise && instance.status.isConnecting) {
  ...
  instance.connectingPromise = new Promise((resolve) => {
    ...
    (async () => {
      try {
        ...
        logBot(accountId, "info", "جاري إنشاء اتصال WASocket جديد...");
        ...
        newSock.ev.on("connection.update", async (update: Partial<ConnectionState>) => {
          ...
          if (connection === "close") {
            ...
            logBot(accountId, "warn", `أُغلق اتصال الواتساب. الرمز: ${statusCode}, تسجيل خروج: ${isLoggedOut}`);
            ...
            if (isLoggedOut || instance.isManualDisconnect) {
              ...
              if (instance.reconnectAttempts > 4) {
                logBot(accountId, "error", "تجاوز حد محاولات إعادة الاتصال. يُرجى طلب رمز QR جديد.");
                try {
                  if (fs.existsSync(sessionDir)) {
                    fs.rmSync(sessionDir, { recursive: true, force: true });
                  }
                ...
                instance.status.isConnecting = false;
                instance.status.error = "انتهت صلاحية جلسة الواتساب القديمة، يرجى طلب رمز QR جديد.";
                safeResolve(getWhatsAppStatus(accountId));
                return;
              }
...
export async function initWhatsAppBot() {
  try {
    ...
    // 3. Connect each account asynchronously
    for (const accId of discoveredAccounts) {
      (async (id: string) => {
        try {
          let hasLocal = hasExistingSession(id);
          if (!hasLocal) {
            console.log(`[WhatsApp Bot (${id})] Checking Supabase cloud backup...`);
            hasLocal = await restoreSessionFromCloud(id);
          }
          if (hasLocal) {
            console.log(`[WhatsApp Bot (${id})] Found session, auto-connecting in background...`);
            connectWhatsAppBot(id).catch((err) =>
              console.error(`[WhatsApp Bot (${id})] Auto-connect failed:`, err)
            );
          }
        } catch (accErr) {
          console.error(`[WhatsApp Bot (${id})] Error initializing account:`, accErr);
        }
      })(accId);
    }
  } catch (e) {
